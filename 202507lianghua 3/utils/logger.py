...
class PerformanceLogHandler(logging.Handler):
    ...
    def __init__(self, metrics_file: str = "logs/performance.json"):
        ...
        self.metrics_file = os.path.join(metrics_dir, f"{log_date}_{base_name}.json")
        
        self.metrics_buffer = []
        self.buffer_size = 100
        self.last_flush = time.time()
        self.flush_interval = 60  # 60秒刷新一次
        self.last_date_check = datetime.now().date()
    ...
    def emit(self, record: logging.LogRecord) -> None:
        """处理性能日志"""
        try:
            # 处理原有的PERFORMANCE级别日志或包含PERFORMANCE关键字的DEBUG日志
            if (record.levelno == LogLevel.PERFORMANCE.value or 
                (record.levelno == logging.DEBUG and "PERFORMANCE" in record.getMessage())):
                metric_data = {
                    'timestamp': record.created,
    ...
    def _flush_metrics(self) -> None:
        ...
        try:
            ...
            if current_date != self.last_date_check:
                log_date = current_date.strftime("%Y%m%d")
                metrics_dir = os.path.dirname(self.metrics_file)
                self.metrics_file = os.path.join(metrics_dir, f"{log_date}_performance.json")
...
class TradingLogger:
    ...
    def __init__(self, log_dir: str = "logs"):
        ...
        trade_handler = logging.handlers.TimedRotatingFileHandler(
            ...
            interval=1,             # 间隔1天
            backupCount=90,         # 保留90天交易日志（更重要）
            encoding='utf-8'
        )
        # 设置交易日志文件名格式为 YYYYMMDD_trades.log
        trade_handler.suffix = "%Y%m%d_trades.log"
        trade_handler.extMatch = re.compile(r"^\d{8}_trades\.log$")
...
class LoggerManager:
    ...
    def _setup_logging(self) -> None:
        ...
        # 设置日志文件名格式为 YYYYMMDD_system.log
        file_handler.suffix = "%Y%m%d_system.log"
        file_handler.extMatch = re.compile(r"^\d{8}_system\.log$")
        ...
        # 设置错误日志文件名格式为 YYYYMMDD_errors.log
        error_handler.suffix = "%Y%m%d_errors.log"  
        error_handler.extMatch = re.compile(r"^\d{8}_errors\.log$")
        ...
        self.performance_handler.setLevel(logging.DEBUG)  # 确保能接收DEBUG级别的性能日志
    ...
    def log_system_performance(self, metric_name: str, value: float, extra: Dict[str, Any] = None) -> None:
        ...
        # 使用DEBUG级别，这样性能日志不会在控制台显示，只保存到性能文件中
        logger.debug(
