...
class MACDDivergenceDetector:
    ...
    def detect_divergence(self, df: pd.DataFrame, symbol: str = "BTCUSDT") -> List[DivergencePattern]:
        """检测背离信号的简化接口"""
        try:
            result = self.detect_divergence_enhanced(df, symbol)
            return [result] if result else []
        except Exception as e:
            self.logger.error(f"检测背离失败: {e}")
            return []
    
    def detect_continuous_divergence(self, df: pd.DataFrame, symbol: str = "BTCUSDT") -> List[DivergencePattern]:
        """连续检测背离信号"""
        try:
            # 使用滑动窗口检测
            results = []
            window_size = min(200, len(df))
            
            if len(df) >= window_size:
                recent_df = df.tail(window_size).copy()
                result = self.detect_divergence_enhanced(recent_df, symbol)
                if result:
                    results.append(result)
            
            return results
        except Exception as e:
            self.logger.error(f"连续检测背离失败: {e}")
            return []
