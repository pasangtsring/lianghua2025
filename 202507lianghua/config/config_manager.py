...
class ConfigManager:
    ...
    def _load_env_file(self) -> None:
        """加载环境变量文件"""
        env_files = ['config/production.env', '.env', '.env.production']
        ...
        for env_file in env_files:
            if Path(env_file).exists():
                try:
                    ...
                    self.logger.warning(f"加载环境变量文件失败 {env_file}: {e}")
    ...
    def _merge_env_vars(self, config_data: Dict[str, Any]) -> None:
        ...
        env_mappings = {
            ...
            'BINANCE_SECRET_KEY': ['api', 'binance', 'api_secret'],
            'BINANCE_API_SECRET': ['api', 'binance', 'api_secret'],  # 兼容旧版本
            ...
            'RISK_PER_TRADE': ['trading', 'risk', 'risk_per_trade'],
            'STOP_LOSS_PCT': ['trading', 'risk', 'stop_loss_pct'],
            'TAKE_PROFIT_RATIO': ['trading', 'risk', 'take_profit_ratio'],
        ...
        for env_var, config_path in env_mappings.items():
            ...
            if value is not None:
                ...
                # 类型转换
                if env_var in ['MAX_WORKERS', 'MAX_LEVERAGE']:
                    value = int(value)
                elif env_var in ['RISK_PER_TRADE', 'STOP_LOSS_PCT', 'TAKE_PROFIT_RATIO']:
                    value = float(value)
